...
gFunMap['button'] = function(c, n) {
    var name = stringify(_eval(c, n.children[1]).eval);
    var func = stringify(_eval(c, n.children[2]).eval);
    $('#telem').html($('#telem').html() + '<button onmousedown=\'eval(parse("(' + func + ')"))\'>' + name + '</button><br>');
}
